...
async function main() {
  console.log('🌱 Checking if quiz questions need seeding...')

  // Check if already seeded
  const existingCount = await prisma.quizQuestion.count()
  
  if (existingCount >= 35) {
    console.log(`✅ Quiz questions already seeded (${existingCount} questions)`)
    console.log('   Skipping seed...')
    return
  }

  console.log(`📊 Found ${existingCount} questions, seeding remaining...`)
  ...
  const questions = [
    ...
    { stepOrder: 2, question: 'Why use Server Components for fetching products?', options: ['Faster client', 'Better SEO and load time', 'Easier state', 'Caching only'], correctIndex: 1, explanation: 'Server Components improve SEO and initial load.', order: 1 },
    ...
    { stepOrder: 3, question: 'Why use Zustand over Context?', options: ['Required', 'Better performance and API', 'TypeScript only', 'Free'], correctIndex: 1, explanation: 'Simpler API, better performance.', order: 1 },
    ...
    { stepOrder: 5, question: 'How do you protect admin routes in Next.js?', options: ['Middleware to check auth', 'Hide nav link', 'CSS', 'Trust users'], correctIndex: 0, explanation: 'Middleware runs before page loads.', order: 1 },
    ...
    { stepOrder: 6, question: 'What is debouncing in search functionality?', options: ['Caching', 'Delaying calls until typing stops', 'Sorting', 'Filtering'], correctIndex: 1, explanation: 'Waits for pause to reduce API calls.', order: 1 },
    ...
    { stepOrder: 7, question: 'What must you do before deploying to production?', options: ['Delete node_modules', 'Set env vars in Vercel', 'Upgrade Pro', 'npm audit'], correctIndex: 1, explanation: 'Configure environment variables.', order: 1 },
  ...
  console.log(`✅ Seeded ${created} new quiz questions`)
  console.log(`📊 Total quiz questions: ${await prisma.quizQuestion.count()}`)
}
